...
int aes_encode(const uint8_t *data, size_t data_len,
               std::unique_ptr<uint8_t[]> &data_out) {
  EVP_CIPHER_CTX *ctx;
  ...
  std::unique_ptr<uint8_t[]> encrypted(
      new uint8_t[16 * ((data_len + 15) / 16 + 2)]);
...
int aes_decode(const uint8_t *data, size_t data_len,
               std::unique_ptr<uint8_t[]> &data_out) {
  EVP_CIPHER_CTX *ctx;
  unsigned char key[32] = "ScherzIndustries";
  unsigned char iv[16] = "012345678901234";
...
void step1_write() {
  ...
  int data_out_len =
      aes_encode(builder.GetBufferPointer(), builder.GetSize(), data_out);
